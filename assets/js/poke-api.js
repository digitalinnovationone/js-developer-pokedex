...
function convertDataPokemons(pokeData) {
...
pokeApi.getDescription = (order) => {
    const url = `https://pokeapi.co/api/v2/pokemon-species/${order}/`
    return fetch(url).then(res => res.json()).then(data => data.flavor_text_entries[1].flavor_text)
}
...
pokeApi.getSinglePokemon = (pokemonId) => {
    ...
    return fetch(url)
        .then((response) => response.json())
        .then((jsonBody) => convertDataPokemons(jsonBody))
        .then(res => res)
}