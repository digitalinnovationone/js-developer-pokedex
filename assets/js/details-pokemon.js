...
nextButton.addEventListener('click', (event) => {
    ...
    if (pokemonId !== null) {
        ...
        console.error('O ID do Pokémon na URL não é válido ou não foi encontrado')
    }
})
...
function convertPokemonToLi(pokemon) {
    return `
           ...
           <h1 class="name">${pokemon.name}  <span class="numPokemon">#${pokemon.numPokemon}</span></h1>
...
pokeApi.getPokemonById = async (pokeId) => {
    ...
    try {
      ...
      navbar.classList.add(pokemon.type)
         ...
         console.error('Falha ao recuperar informações detalhadas do Pokémon:', error)
    }
  }
