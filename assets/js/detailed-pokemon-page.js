...
pokemonDetailedPage.getPokemonDetail = (pokemonName) => {
    ...
    return fetch(url)
        ...
        .then((pokemon) => {
            ...
            document.getElementById('detailed-content').classList.add(pokemon.types[0]);
            document.getElementById('peso').innerHTML = pokemon.weight;
            document.getElementById('altura').innerHTML = pokemon.height;
            document.getElementById('habilidades').innerHTML = pokemon.abilities.map((ability) => `<div>${ ability }</div>`).join('');
            document.getElementById('imagem').innerHTML = `<img src=" ${ pokemon.photo2 }">`;
            pokemonDetailedPage.getMorePokemonDetail(pokemon);
            pokemonDetailedPage.getNumberFormated(pokemon);
